...
summary: 'Sie können Trident mit  `tridentctl` installieren. Dieser Prozess gilt für Installationen, bei denen die Container-Images, die von Trident benötigt werden, entweder in einer privaten Registry gespeichert sind oder nicht.' 
---
...
NOTE: Falls das Installationsprogramm nicht erfolgreich abgeschlossen wird oder  `trident-controller-<generated id>` (`trident-csi-<generated id>` in Versionen vor 23.01) keinen *Running*-Status hat, wurde die Plattform nicht installiert. Verwenden Sie  `-d` , um link:../troubleshooting.html#troubleshooting-an-unsuccessful-trident-deployment-using-tridentctl["Debug-Modus aktivieren"] das Problem zu beheben und zu analysieren.
