...
.Schritte
. Fügen Sie das kube-state-metrics Helm-Chart hinzu. Zum Beispiel:
+
[source, console]
----
...
prometheus:
  monitor:
    enabled: true
----
. Installieren Sie kube-state-metrics, indem Sie das Helm-Chart bereitstellen. Zum Beispiel:
+
[source, console]
----
...
.Schritte
. Integrieren Sie kube-state-metrics in Prometheus. Bearbeiten Sie die Prometheus-Konfigurationsdatei (`prometheus.yaml` und fügen Sie die Informationen zum kube-state-metrics-Service hinzu. Zum Beispiel:
+
