...
** Trident unterstützt SMB-Volumes nur, wenn sie in Pods eingebunden sind, die auf Windows-Knoten ausgeführt werden. Weitere Informationen finden Sie unter link:../trident-use/trident-fsx-storage-backend.html#prepare-to-provision-smb-volumes["Bereiten Sie die Bereitstellung von SMB-Volumes vor"].
...
NOTE: Wenn Sie eine IAM-Rolle für Trident angeben, können Sie das explizite Angeben der Felder  `apiRegion`,  `apiKey` und  `secretKey` für Trident weglassen. Weitere Informationen finden Sie unter link:../trident-use/trident-fsx-examples.html["FSx for ONTAP Konfigurationsoptionen und Beispiele"].
